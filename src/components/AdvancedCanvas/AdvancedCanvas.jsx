...
function AdvancedCanvas(
  ...
  const docToDisplayPoint = (point) => {
    const { width: docWidth, height: docHeight } = docSizeRef.current;
    const scaleX = displaySize.width / docWidth;
    const scaleY = displaySize.height / docHeight;
  ...
  const renderCanvas = (preview = null) => {
    ...
    ctx.drawImage(contentCanvas, 0, 0, displaySize.width, displaySize.height);

    if (preview) {
      const start = docToDisplayPoint(preview.start);
      const current = docToDisplayPoint(preview.current);
      const scale = (displaySize.width / docSizeRef.current.width + displaySize.height / docSizeRef.current.height) / 2;
  ...
  const applyEyedropper = (point) => {
    ...
    const pixel = ctx.getImageData(
      clamp(Math.floor(point.x), 0, exportCanvas.width - 1),
      clamp(Math.floor(point.y), 0, exportCanvas.height - 1),
      1,
      1
    ).data;
  ...
  const beginInteraction = (e) => {
    ...
    if (tool === 'fill') {
      fillCanvas(canvas, point.x, point.y);
      renderCanvas();
      commitHistory();
      return;
    }
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      if (initialImage) {
        await loadImageToCanvas(contentCanvasRef.current, initialImage, 'stretch');
      }
