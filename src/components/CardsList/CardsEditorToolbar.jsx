...
import { FiUpload, FiCornerUpLeft, FiCornerUpRight, FiPlus } from 'react-icons/fi';
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/50 px-3 py-1.5 shadow-sm">
      <div className="flex w-full flex-col gap-2 overflow-x-auto pb-0.5">
        ...
        <div className="flex min-w-max items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-xl border border-slate-100 bg-slate-50 p-1">
            <button
              ...
              onPointerDown={(e) => {
                e.preventDefault();
                if (activeCanvasRef && activeStatus.canUndo) activeCanvasRef.undo?.();
              }}
            ...
            <button
              ...
              onPointerDown={(e) => {
                e.preventDefault();
                if (activeCanvasRef && activeStatus.canRedo) activeCanvasRef.redo?.();
              }}
            ...
            {BRUSH_TYPES.map((item) => {
              ...
              return (
                <button
                  ...
                  onPointerDown={(e) => {
                    e.preventDefault();
                    setToolbox((prev) => ({ ...prev, brushType: item.id, tool: 'brush' }));
                  }}
                  onClick={() =>
                    setToolbox((prev) => ({ ...prev, brushType: item.id, tool: 'brush' }))
                  }
            ...
            {TOOL_LIST.filter((item) => item.id !== 'brush' && item.id !== 'draw').map((item) => {
              ...
              return (
                <button
                  ...
                  onPointerDown={(e) => {
                    e.preventDefault();
                    setToolbox((prev) => ({ ...prev, tool: item.id }));
                  }}
          ...
          <button
            ...
            onPointerDown={(e) => {
              e.preventDefault();
              if (canAddCard) handleAddCardPair();
            }}
        ...
        {/* HÀNG 2: Color, Size, Opacity (FIX CHO APPLE PENCIL ĐÂY) */}
        <div className="flex w-full min-w-max items-center gap-2">
          ...
          <div className="flex shrink-0 items-center gap-2 rounded-lg border border-slate-100 bg-slate-50/50 px-2 py-1">
            ...
            <input
              type="color"
              value={toolbox.color}
              onChange={(e) => setToolbox((prev) => ({ ...prev, color: e.target.value }))}
              onPointerDown={(e) => {
                e.stopPropagation();
                // Ép trình duyệt mở bảng màu nếu dùng bút
                if (e.pointerType === 'pen') e.currentTarget.click();
              }}
              disabled={isEraser}
              className={cn("color-picker-soft h-5 w-5 rounded bg-transparent touch-none", isEraser ? "opacity-30 cursor-not-allowed" : "cursor-pointer")}
            />
          </div>
          ...
          <div className="flex flex-1 items-center gap-2 rounded-lg border border-slate-100 bg-slate-50/50 px-2 py-1">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setToolbox((prev) => isEraser ? { ...prev, eraserSize: val } : { ...prev, size: val });
              }}
              onPointerDown={(e) => {
                e.stopPropagation();
                // Khóa con trỏ vào thanh trượt khi đặt bút xuống
                if (e.pointerType === 'pen') e.currentTarget.setPointerCapture(e.pointerId);
              }}
              onPointerMove={(e) => {
                // Tự tính toán thay đổi khi bút kéo qua lại
            ...
            />
            <span className="w-[24px] text-right text-[11px] font-bold text-slate-600">{currentSize}</span>
          </div>

          <div className="flex shrink-0 items-center gap-2 rounded-lg border border-slate-100 bg-slate-50/50 px-2 py-1">
            <label className="text-[9px] font-bold uppercase tracking-widest text-slate-500">Opacity</label>
            <input
              type="range"
              min="0.1"
              max="1"
              ...
              onChange={(e) => setToolbox((prev) => ({ ...prev, opacity: Number(e.target.value) }))}
              onPointerDown={(e) => {
                e.stopPropagation();
                if (e.pointerType === 'pen') e.currentTarget.setPointerCapture(e.pointerId);
              }}
              onPointerMove={(e) => {
                if (e.pointerType === 'pen' && e.buttons > 0) {
                  ...
                  setToolbox((prev) => ({ ...prev, opacity: rounded }));
                }
              }}
              disabled={isEraser}
              className={cn("range-soft w-24 touch-none", isEraser && "opacity-30 cursor-not-allowed")}
            />
